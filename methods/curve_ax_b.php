...
if (isset($_POST['solve'])) {
    ...
    if (!empty($_POST['n'])) {
        ...
        try {
            for ($i = 0; $i < $n; $i++) {
                $sumX = $sumX + log($x_array[$i]);
                $sumX2 = $sumX2 + log($x_array[$i]) * log($x_array[$i]);
                $sumY = $sumY + log($y_array[$i]);
                $sumXY = $sumXY + log($x_array[$i]) * log($y_array[$i]);
...
?>
<h4 class="text-primary">Curve Fitting <br> y = ax<sup>b</sup></h4>
